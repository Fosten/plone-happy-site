...
2. Go to your repository at [fosten/plone-happy-site](https://github.com/fosten/plone-happy-site).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `happybaseball.com` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | happybaseball.com | The hostname or IP address of your Docker Swarm manager. |
...
- [Backend Tests Workflow](https://github.com/fosten/plone-happy-site/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/fosten/plone-happy-site/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/fosten/plone-happy-site-backend`) and Frontend (`ghcr.io/fosten/plone-happy-site-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of happybaseball.com](https://github.com/fosten/plone-happy-site/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/happybaseball.com.yml` stack.
